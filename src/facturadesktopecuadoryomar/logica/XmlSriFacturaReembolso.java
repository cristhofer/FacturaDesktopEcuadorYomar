...
public class XmlSriFacturaReembolso
  extends XmlSriAbstratc
{
  ...
  public XmlSriFacturaReembolso(FacturacionIntermedia carga) throws Exception {
    super(carga);
  }
  ...
  public String getInfoDocumento() throws FormatoCargaException {
    ...
    try {
      ...
      sb.append(XmlBuilder.createTag(
        ...
        ) //this.empresaFacade.buscarEstablecimiento(carga.getCodEstab()).getDireccionEstab()
      );
    } catch (Exception ex) {
    ...
    List<String> detalleFacturaNormal = this.carga.getListaInformacion().stream().filter(d -> d.split("\\|")[28].trim().equals("N")).collect(Collectors.toList());
    String datoCarga = detalleFacturaNormal.get(0);
    ...
    List<String> detalleFacturaReembolso = this.carga.getListaInformacion().stream().filter(d -> d.split("\\|")[28].trim().equals("R")).collect(Collectors.toList());
    String[] datosRem = ((String)detalleFacturaReembolso.get(0)).split("\\|");
    sb.append(XmlBuilder.createTag("codDocReembolso", datosRem[0]));
    sb.append(XmlBuilder.createTag("totalComprobantesReembolso", datosRem[1]));
    sb.append(XmlBuilder.createTag("totalBaseImponibleReembolso", datosRem[2]));
    sb.append(XmlBuilder.createTag("totalImpuestoReembolso", datosRem[3]));
  ...
  private String getDetalle() throws FormatoCargaException {
    ...
    List<String> detalleFacturaNormal = this.carga.getListaInformacion().stream().filter(d -> d.split("\\|")[28].trim().equals("N")).collect(Collectors.toList());
    for (String c : detalleFacturaNormal) {
    ...
    System.out.println("SUMA: " + sumaPrecioTSI.toString() + ", IMPUESTO: " + sumaPrecioVT.toString() + " , VALOR TOTAL: " + valorTotal.toString());

    
    if (sumaPrecioTSI.doubleValue() != precioTotalSinImpuesto.doubleValue()) {
      throw new FormatoCargaException(cargaInicial, "totalSinImpuestos", "ERROR EN DIFERENCIAS EN TOTAL SIN IMPUESTOS", "msg");
    }
  ...
  private String getReembolso() throws FormatoCargaException {
    StringBuilder sb = new StringBuilder();
    sb.append(XmlBuilder.createOpenTag("reembolsos"));
    List<String> detalleFacturaReembolso = this.carga.getListaInformacion().stream().filter(d -> d.split("\\|")[28].trim().equals("R")).collect(Collectors.toList());
    for (String c : detalleFacturaReembolso) {
      String[] rem = c.split("\\|");
      sb.append(XmlBuilder.createOpenTag("reembolsoDetalle"));
      sb.append(XmlBuilder.createTag("tipoIdentificacionProveedorReembolso", rem[4]));
      sb.append(XmlBuilder.createTag("identificacionProveedorReembolso", rem[5]));
      sb.append(XmlBuilder.createTag("codPaisPagoProveedorReembolso", rem[6]));
      sb.append(XmlBuilder.createTag("tipoProveedorReembolso", rem[7]));
      sb.append(XmlBuilder.createTag("codDocReembolso", rem[18]));
      sb.append(XmlBuilder.createTag("estabDocReembolso", rem[16]));
      sb.append(XmlBuilder.createTag("ptoEmiDocReembolso", rem[17]));
      sb.append(XmlBuilder.createTag("secuencialDocReembolso", rem[8]));
      sb.append(XmlBuilder.createTag("fechaEmisionDocReembolso", rem[9]));
      sb.append(XmlBuilder.createTag("numeroautorizacionDocReemb", rem[10]));
      
      sb.append(XmlBuilder.createOpenTag("detalleImpuestos"));
      sb.append(XmlBuilder.createOpenTag("detalleImpuesto"));
      sb.append(XmlBuilder.createTag("codigo", rem[11]));
      sb.append(XmlBuilder.createTag("codigoPorcentaje", rem[12]));
      sb.append(XmlBuilder.createTag("tarifa", rem[13]));
      sb.append(XmlBuilder.createTag("baseImponibleReembolso", rem[14]));
      sb.append(XmlBuilder.createTag("impuestoReembolso", rem[15]));
      sb.append(XmlBuilder.createCloseTag("detalleImpuesto"));
      sb.append(XmlBuilder.createCloseTag("detalleImpuestos"));
      sb.append(XmlBuilder.createCloseTag("reembolsoDetalle"));
    } 
    sb.append(XmlBuilder.createCloseTag("reembolsos"));
