...
public class DocumentoProveedorFacade
{
  private DocumentoRecibidoFacade documentoLogica = new DocumentoRecibidoFacade();
  private ConexionSql conexion;
  
  public DocumentoProveedorFacade() {
  ...
  public void actualizarDocumentoProveedor(DocumentosRecibidos dr) {
    ...
    try {
      ...
      guardarDetalleRetencion(infoRet, dr);
      this.conexion.commitTransaction();
      this.conexion.closeConnection();
      dr.setEstadoClienteDocu("E");
      this.documentoLogica.modificar(dr);
    } catch (SQLException ex) {
      Logger.getLogger(DocumentoProveedorFacade.class.getName()).log(Level.SEVERE, null, ex);
      try {
        this.conexion.rollbackTransaction();
        this.conexion.closeConnection();
      } catch (SQLException ex1) {
        Logger.getLogger(DocumentoProveedorFacade.class.getName()).log(Level.SEVERE, null, ex1);
        this.conexion.closeConnection();
      } 
    } 
  }
  ...
  private InfoRetencion getListadoInfoRetencion(DocumentosRecibidos dr) {
    ...
    nodo.setValue(nodo.getValue().replace(" ", ""));
    nodo.setValue(nodo.getValue().replace("\n", ""));
    nodo.setValue(nodo.getValue().replace("\t", ""));
    nodo.setValue(nodo.getValue().replace("\r", ""));
